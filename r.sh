...
# Buat /etc/fstab secara manual (tanpa genfstab)
cat > /etc/fstab <<EOT
# /etc/fstab - Generated by LeakOS Installer (BIOS/MBR only)

UUID=$ROOT_UUID   /          ext4    defaults,relatime,errors=remount-ro   0 1
tmpfs             /tmp       tmpfs   defaults,noatime,mode=1777            0 0

# Tambahkan baris lain jika perlu (swap, /home, dll) nanti manual
EOT
